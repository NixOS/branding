...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="*">
    ...
    <xsl:call-template name="pad">
      <xsl:with-param name="depth" select="$padsp"/>
    </xsl:call-template>
    <xsl:text>&lt;</xsl:text>
  ...
  <xsl:template name="emit-points-lines">
    ...
    <xsl:if test="$sn != ''">
      ...
      <xsl:text>[</xsl:text>
      <xsl:value-of select="format-number($idx, '0000')"/>
      <xsl:text>] </xsl:text>
  ...
  <xsl:template name="emit-linearGradient-xy">
    ...
    <xsl:if test="@x1">
      ...
      <xsl:text>[x1] @x1: </xsl:text>
      <xsl:value-of select="normalize-space(@x1)"/>
      <xsl:text>
</xsl:text>
    </xsl:if>
    <!-- x2 -->
    <xsl:if test="@x2">
      ...
      <xsl:text>[x2] @x2: </xsl:text>
      <xsl:value-of select="normalize-space(@x2)"/>
      <xsl:text>
</xsl:text>
    </xsl:if>
    <!-- y1 -->
    <xsl:if test="@y1">
      ...
      <xsl:text>[y1] @y1: </xsl:text>
      <xsl:value-of select="normalize-space(@y1)"/>
      <xsl:text>
</xsl:text>
    </xsl:if>
    <!-- y2 -->
    <xsl:if test="@y2">
      ...
      <xsl:text>[y2] @y2: </xsl:text>
      <xsl:value-of select="normalize-space(@y2)"/>
      <xsl:text>
</xsl:text>
    </xsl:if>
  </xsl:template>
